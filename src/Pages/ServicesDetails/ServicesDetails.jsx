...
import backgroundImage from '/src/assets/img/SUV-blue.svg';
...
const ServicesDetails = () => {
  ...
  const handleBookService = async(event) => {
    ...
    try {
      const res = await axiosSecure.post("/appointments", booking);
      console.log(res);
      if (res.data.insertedId) { // Ensure to access data property
          // alert('Appointment booked Successfully');
          Swal.fire({
              title: "Good job!",
              text: "Your Appointment has been added",
              icon: "success"
          });
      }
  } catch (error) {
      ...
      Swal.fire({
          title: "Error!",
          text: "There was an issue booking your appointment.",
          icon: "error"
      });
  } finally {
          ...
          <div className='flex-1 p-4 bg-gradient-to-b from-slate-300 to-transparent'>
            <form onSubmit={handleBookService} className="card-body">
              ...
              <div className="form-control mt-6">
                {
                  user ? (
                    <button className="btn bg-mainColor text-white hover:bg-black" type='submit' disabled={loading}>
                      {loading ? 'Booking...' : 'Book Appointment'}
                    </button>
                  ) :
                    <Link to="/login">
                      <button className="btn w-full bg-mainColor text-white hover:bg-black" type='submit'>Login for Appointment</button>
                    </Link>
